...
@RequestMapping({"${platform.path:}/rest", "${platform.path:}/public/rest","${platform.path:}/bizrest", "${platform.path:}/public/bizrest"})
...
public class RestApiController {
    ...
    void init(){
        ...
        if(ObjectUtil.isNotEmpty(dataSource)){
            ...
            String url = SpringUtil.getProperty("spring.datasource.url");//"jdbc:mysql://localhost:3307/db_qixing_v2?serverTimezone=GMT%2B8&useSSL=false";
            String username = SpringUtil.getProperty("spring.datasource.username");//"root";
            String password = SpringUtil.getProperty("spring.datasource.password");//"mysqladmin";
...
//            Db.init(main,dataSource);
    ...
    @RequestMapping(path = {"/{entityName}/list"}, produces = "application/json")
    ...
    @RequestMapping(path = {  "/{entityName}/page" }, produces = "application/json")
    //@ApiOperation(value = "返回实体数据列表", notes = "page与size同时大于零时返回分页实体数据列表,否则返回全部数据列表;
    ...
    @RequestMapping(path = {"/{entityName}/tree"}, produces = "application/json")
    //@ApiOperation(value = "返回实体数据列表", notes = "page与size同时大于零时返回分页实体数据列表,否则返回全部数据列表;
...
//    }
    ...
    public Result delete(@PathVariable("entityName") String entityName,/*@PathVariable("ids") String ids,*/ HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            Object idVal = parameters.get(primaryKey);
            Object[] argsDelete = new Object[]{idVal} ;
    ...
    @RequestMapping(path = {"/{entityName}"}, produces = "application/json")
...
//    @RequestMapping(path = {"/{entityName}/edit"}, produces = "application/json")
...
//    }
    ...
    public Result update(@PathVariable("entityName") String entityName, HttpServletRequest request) {
        ...
        try {
            ...
            Record record = new Record();
            List args = RestUtil.getPrimaryKeyArgs(parameters, table);
            record = Db.findById(tableName, primaryKey, primaryValue/*args.toArray()*/);
