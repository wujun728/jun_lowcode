...
package io.github.wujun728.sys.modular.app.service.impl;
...
import io.github.wujun728.sys.modular.app.enums.SysAppExceptionEnum;
import io.github.wujun728.sys.modular.app.mapper.SysAppMapper;
...
public class SysAppServiceImpl extends ServiceImpl<SysAppMapper, SysApp> implements SysAppService {
    ...
    public List<Dict> getLoginApps(Long userId, List<Long> roleIdList) {
        ...
        this.list(queryWrapper).forEach(sysApp -> {
            ...
            if (YesOrNotEnum.Y.getCode().equals(sysApp.getActive())) {
                ...
                dict.put(CommonConstant.ACTIVE, true);
                //将其放在第一个
                userAppDictList.add(0, dict);
            } else {
                dict.put(CommonConstant.ACTIVE, false);
    ...
    public PageResult<SysApp> page(SysAppParam sysAppParam) {
        ...
        if (ObjectUtil.isNotNull(sysAppParam)) {
            ...
            if (ObjectUtil.isNotEmpty(sysAppParam.getName())) {
                queryWrapper.lambda().like(SysApp::getName, sysAppParam.getName());
            }
            //根据编码模糊查询
            if (ObjectUtil.isNotEmpty(sysAppParam.getCode())) {
                queryWrapper.lambda().like(SysApp::getCode, sysAppParam.getCode());
            }
            //排序
            if(ObjectUtil.isAllNotEmpty(sysAppParam.getSortBy(), sysAppParam.getOrderBy())) {
                queryWrapper.orderBy(true, sysAppParam.getOrderBy().equals(CommonConstant.ASC), StrUtil.toUnderlineCase(sysAppParam.getSortBy()));
            }
        }
        queryWrapper.lambda().eq(SysApp::getStatus, CommonStatusEnum.ENABLE.getCode());
    ...
    @Override
    public void delete(List<SysAppParam> sysAppParamList) {
        sysAppParamList.forEach(sysAppParam -> {
    ...
    private void checkParam(SysAppParam sysAppParam, boolean isExcludeSelf) {
        ...
        long countByName = this.count(appQueryWrapperByName);
        long countByCode = this.count(appQueryWrapperByCode);
        long countByActive = this.count(appQueryWrapperByActive);
